...
bool twitCurl::userSearch(const std::string& searchQuery, const std::string resultCount)
{
	/* Prepare URL */
	//std::string buildUrl = "https://api.twitter.com/1.1/users/search.json?q=" + searchQuery + "&page=1&count=" + resultCount;

	//test
	std::string buildUrl = twitCurlDefaults::TWITCURL_PROTOCOLS[m_eProtocolType] +
		twitterDefaults::TWITCURL_BASE_URL + "users/search" +
		twitCurlDefaults::TWITCURL_EXTENSIONFORMATS[m_eApiFormatType] +
		twitCurlDefaults::TWITCURL_URL_SEP_QUES + twitCurlDefaults::TWITCURL_SEARCHQUERYSTRING +
		urlencode(searchQuery);
	/* Perform GET */
...
bool twitCurl::userLookup( const std::vector<std::string> &userInfo, const bool isUserId )
{
    ...
    std::string userIds = "";
    std::string sep = "";
    for( unsigned int i = 0 ; i < std::fmin((size_t)100, userInfo.size()); i++, sep = "," )
    {
...
/*++
* @method: twitCurl::suggestionsGetSlugs
*
* @description: method to get user suggestion slugs
*
* @input: lang - Restrict suggestions to a given language
*
* @output: true if GET is success, otherwise false. This does not check http
*          response by twitter. Use getLastWebResponse() for that.
*
*--*/
bool twitCurl::suggestionsGetSlugs(const std::string lang) {
	/* Prepare URL */
	std::string buildUrl = twitCurlDefaults::TWITCURL_PROTOCOLS[m_eProtocolType] +
		twitterDefaults::TWITCURL_USER_SUGGESTIONS_URL +
		twitCurlDefaults::TWITCURL_EXTENSIONFORMATS[m_eApiFormatType];

	if (lang.length() > 0)
	{
		buildUrl += twitCurlDefaults::TWITCURL_URL_SEP_QUES + twitCurlDefaults::TWITCURL_LANG + lang;
	}
...
/*++
* @method: twitCurl::suggestionsGet
*
* @description: method to get user suggestion based on a slug
*
* @input: slug - Slug on which suggestions are based
*		  lang - Restrict results to this language	
*
* @output: true if GET is success, otherwise false. This does not check http
*          response by twitter. Use getLastWebResponse() for that.
*
*--*/
bool twitCurl::suggestionsGet(const std::string slug, const std::string lang) {
	/* Prepare URL */
	std::string buildUrl = twitCurlDefaults::TWITCURL_PROTOCOLS[m_eProtocolType] +
		twitterDefaults::TWITCURL_USER_SUGGESTIONS_URL +
		twitCurlDefaults::TWITCURL_SLUG + slug +
		twitCurlDefaults::TWITCURL_EXTENSIONFORMATS[m_eApiFormatType];
...
*
*--*/
bool twitCurl::blockCreate( const std::string& userInfo, const bool isUserId )
{
        /* Prepare URL */
        std::string buildUrl = twitCurlDefaults::TWITCURL_PROTOCOLS[m_eProtocolType] +
        ...
        std::string postData = "user_id=" + urlencode(userInfo);

        if (!isUserId) {
            postData = "screen_name=" + urlencode(userInfo);
        }
...
*
*--*/
bool twitCurl::blockDestroy(const std::string& userInfo, const bool isUserId)
{
    /* Prepare URL */
    std::string buildUrl = twitCurlDefaults::TWITCURL_PROTOCOLS[m_eProtocolType] +
...
{
    /* Perform GET */
	return performGet("https://api.twitter.com/1.1/trends/place.json?id=23424829");
}

bool twitCurl::trendsGetInternational(const std::string& id) {
	return performGet("https://api.twitter.com/1.1/trends/place.json?id=" + id);
}
